...
//: Если вы нажмёте на символ *"Глаза"* в правой части вашего Playground (там, где генерируется вывод информации об объектах), то вы увидите, что элементы в Словаре находятся вовсе не в том порядке, в котором они были заданы изначально.
...
dropChance.isEmpty
dropChance.first // Не рекомендуется использовать, поскольку порядок элементов в Словаре меняется.
