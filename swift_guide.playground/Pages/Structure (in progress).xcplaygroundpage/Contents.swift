...
var knight = Knight(berserkStatus: false, awakeningStatus: true)
...
struct BananaHat {
  ...
  // а какие пердоставить на изменение создателю объекта Структуры
...
// мы уже не сможем задать их при объекта Структуры (за исключением defense)
...
struct Stunner {
  ...
  // Если вдруг нам в инициализатор попадёт отрицательное число - программа не упадёт с ошибкой,
