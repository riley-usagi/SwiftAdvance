...
//: Однако, при изменении значения у первого объекта, значение второго остаётся неизменным.
...
*/
//: В случае же с Классами - всё несколько иначе.
/// Пример Класса
class Rapier {
  var element: String
...
/*:
> Вот здесь мы уже видим разницу. При изменении значения одного объекта, так же изменилось и значение другого.
...
//: > Случается такое, что вам необходимо отключить возможность наследования от Класса. Делается это довольно просто.
...
//: ### Деиницилизация
//: > В отличие от Структур, у Классов помимо *конструкторов* есть ещё и *деструкторы*.
//: *Деструктор* не удалает объект из памяти. Этим занимается сама среда выполнения. *Деструктор* лишь вызывается в момент удаления объекта из памяти.
/// Бутыль с синей магической жидкостью
class BluePotion {
  var action: String = "Восстанавливает очки SP"
  
  // Вызывется, когда объект будет удалён из памяти.
  deinit {
